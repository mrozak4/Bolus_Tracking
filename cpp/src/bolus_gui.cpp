...
bool BolusApp::load_dataset(const std::string& csv_path) {
        ...
        // Fallback to the original stem if the unregistered image isn't available
        if (!std::filesystem::exists(m_tiff_path)) {
            stem = orig_stem;
            m_tiff_path = (parent / (stem + ".tif")).string();
            if (!std::filesystem::exists(m_tiff_path)) {
        ...
        std::string tiff_name = std::filesystem::path(m_tiff_path).filename().string();
        std::string roi_name = std::filesystem::path(m_rois_path).filename().string();
