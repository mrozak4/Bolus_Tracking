...
* **Bolus Tracking Studio** : Le **Bolus Tracking Studio** natif C++ (`cpp/src/bolus_gui.cpp`) est une application de bureau mono-binaire construite avec Dear ImGui, ImPlot et GLFW. Il offre une interface de triage moderne pour le chargement TIFF, l'ajustement et le rendu interactif des graphiques. Les opérateurs peuvent réviser, filtrer (PASS/WARN/FAIL/REVIEW/STALL), trier par qualité d'ajustement (N° de ROI, sévérité QC ou RCB), réajuster manuellement (marqueurs de début, pic et fin de bolus), surcharger le statut QC (Forcer CONFORME/ÉCHEC/STAGNATION), rogner les fenêtres d'ajustement et sauvegarder directement — le tout avec le thème sombre MCM et la localisation en 44 langues.
* **Dorsal unique / Grande portabilité** : Le dorsal C++ peut être compilé sur Windows, Linux et macOS. L'application native ne nécessite que cmake et les dépendances natives (Eigen, libtiff, GLFW).
